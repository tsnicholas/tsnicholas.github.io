...
export default function render(data, type) {
    const renderStrategy = renderStrategies[type];
    if (!renderStrategy) {
        throw new Error(`Strategy type ${type} not found.`);
    }

    const context = document.getElementById(`${type}-container`);
    renderStrategy(data, context);
}
